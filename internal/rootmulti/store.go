...
func (rs *Store) handlePruning(version int64) error {
	...
	return rs.PruneStores(0, true, nil)
}
...
func (rs *Store) PruneStores(batch int, clearPruningManager bool, pruningHeights []int64) (err error) {
	...
	for key, store := range rs.stores {
		...
		if batch == 0 {
			batch = len(pruningHeights)
		}

		for i := 0; i < len(pruningHeights); i += batch {
			j := i + batch
			if j > len(pruningHeights) {
				j = len(pruningHeights)
			}

			err := store.(*iavl.Store).DeleteVersions(pruningHeights[i:j]...)
			if err == nil {
				continue
			}
